...
def inspect_sample():
    ...
    with torch.no_grad():
        ...
        ys_heun = sdeint(sde_strat, y0=y0, ts=ts, dt=dt, bm=bm, method='heun')
        ...
        ts_, ys_euler_, ys_heun_, ys_analytical_ = to_numpy(
            ts, ys_euler, ys_heun, ys_analytical)

    # Visualize sample path.
    img_dir = os.path.join('.', 'diagnostics', 'plots', 'stratonovich_diagonal')
    makedirs_if_not_found(img_dir)

    for i, (ys_euler_i, ys_heun_i, ys_analytical_i) in enumerate(
            zip(ys_euler_, ys_heun_, ys_analytical_)):
        plt.figure()
...
def inspect_strong_order():
    ...
    with torch.no_grad():
        ...
        for dt in tqdm.tqdm(dts):
            ...
            _, ys_heun = sdeint(sde_strat, y0=y0, ts=ts, dt=dt, bm=bm, method='heun')
            ...
            euler_mse_, heun_mse_ = to_numpy(euler_mse, heun_mse)

            euler_mses_.append(euler_mse_)
            heun_mses_.append(heun_mse_)
    del euler_mse_, heun_mse_
