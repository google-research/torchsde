...
def inspect_sample():
    ...
    with torch.no_grad():
        ...
        ts_, ys_heun_, ys_midpoint_, ys_milstein_strat_, ys_mil_strat_grad_free_, ys_analytical_ = to_numpy(
            ts, ys_heun, ys_midpoint, ys_milstein_strat, ys_mil_strat_grad_free, ys_analytical)

    # Visualize sample path.
    img_dir = os.path.join('.', 'diagnostics', 'plots', 'stratonovich_diagonal')
    makedirs_if_not_found(img_dir)

    for i, (ys_heun_i, ys_midpoint_i, ys_milstein_strat_i, ys_mil_strat_grad_free_i, ys_analytical_i) in enumerate(
            zip(ys_heun_, ys_midpoint_, ys_milstein_strat_, ys_mil_strat_grad_free_, ys_analytical_)):
        plt.figure()
...
def inspect_strong_order():
    ...
    del heun_mse_, midpoint_mse_, milstein_strat_mse_, mil_strat_grad_free_mse_
